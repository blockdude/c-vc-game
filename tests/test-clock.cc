...
{
    FixedClock fc = fixed_clock_create(60.0);
    EXPECT_TRUE(std::fabs(fc.interval - 1.0 / 60.0) < 0.0001);
}

UTEST(clock, fixed_create_zero_rate)
{
    FixedClock fc = fixed_clock_create(0.0);
    EXPECT_TRUE(std::fabs(fc.interval - 1.0 / 60.0) < 0.0001);
}

UTEST(clock, fixed_set_rate)
{
    FixedClock fc = fixed_clock_create(30.0);
...
{
    FrameClock fc = frame_clock_create(60.0);
    ClockConfig cfg = clock_config_default();
    ClockStats s = { 0 };
